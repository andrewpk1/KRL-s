ruleset trip_store {
	...
	rule collect_trips{
		...
		always{
      		...
      		ent:trip_id := ent:trip_id.defaultsTo(ent:clear_id,"initializing trip_ids");
	...
	rule collect_long_trips{
		...
		always{
			...
			ent:trip_id := ent:trip_id.defaultsTo(ent:clear_id, "initializing trip_ids");
			ent:long_trips{[ent:trip_id["long_trip_id"],"mileage"]} := passed_mileage;
			ent:long_trips{[ent:trip_id["long_trip_id"],"timestamp"]} := timestamp;
			ent:long_trip_id{["long_trip_id"]} := ent:long_trip_id{["long_trip_id"]} + 1
		}
	}
	rule clear_trips{
